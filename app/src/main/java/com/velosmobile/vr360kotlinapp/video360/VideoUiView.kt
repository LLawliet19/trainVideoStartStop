...
import com.velosmobile.vr360kotlinapp.Constant.diceval
import com.velosmobile.vr360kotlinapp.Constant.pos
...
class VideoUiView
    ...
    private inner class UiUpdater : SurfaceTexture.OnFrameAvailableListener {
        ...
        private val uiThreadUpdater = Runnable {
            ...
            var positionMs = player.currentPosition
            seekBar.progress = positionMs
            if(city == pos){
                cities.get(pos)?.let { mediaPlayer?.seekTo(it) }
                Log.d(positionMs.toString(), "done2")
                ...
                if (positionMs in ((diceval * 4000 + 2000 + cities.get(pos)!!) % 65800)..((500 + diceval * 4000 + 2000 + cities.get(pos)!!) % 65800)) {
